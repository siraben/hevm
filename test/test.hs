...
tests = testGroup "hevm"
  ...
  , testGroup "StorageTests"
    ...
    , testCase "accessStorage uses fetchedStorage" $ do
        ...
        vm1 <- stToIO $ execStateT (EVM.accessStorage 0 (Lit 0) (pure . pure ())) vm
        ...
        vm4 <- stToIO $ execStateT (EVM.accessStorage 0 (Lit 0) (pure . pure ())) vm3
      ...
      testCase "summary storage writes" $ do
        ...
        testCase "multiple-contracts" $ do
          ...
          (_, [Cex (_, cex)]) <- withSolvers Z3 1 Nothing $ \s -> do
            vm0 <- stToIO $ abstractVM (mkCalldata (Just (Sig "call_A()" [])) []) c Nothing AbstractStore False
            let vm = vm0
        ...
        ignoreTest $ testCase "safemath distributivity (yul)" $ do
          ...
          vm <- stToIO $ abstractVM (mkCalldata (Just (Sig "distributivity(uint256,uint256,uint256)" [AbiUIntType 256, AbiUIntType 256, AbiUIntType 256])) []) yulsafeDistributivity Nothing AbstractStore False
